---
layout: post
title:  "Angular performant drag and drop"
date:   2022-11-25
published: false
comments: true
categories: Angular 
...
<iframe href="https://angular-ivy-heyrwg.stackblitz.io/">
</iframe>
...
It is important to realise why we are listening to the `mousemove` and `mouseup` events of the `document` and not of our native element of the thing we want to drag:
...
Creating a `dragMove$` observable can be done to leveraging the `switchMap`, `map`, `tap` and `takeUntil` operators:
...
We can see the basic flow in the code sample below. We haven't implemented the calculation of the position nor the update position functionality,
...
We need to subtract the offset of the startEvent from the moveEvent. If we do that,
...
```
If we understand how Angular Change Detection works, we realise that every `mousedown`, `mouseup` but also
...
If you don't know how Change Detection works, you can download this [free Cheat Sheet](https://blog.simplified.courses/angular-change-detection-cheat-sheet-explained/), or learn about it in this 
[Angular Change Detection Simplified book](https://www.simplified.courses/angular-change-detection-simplified-e-book).
...
After all we are updating the position with vanillajs. The only thing that will break is the `dragging` class.
...
We could optimize this by detaching from the ChangeDetectorRef and trigger `detectChanges()` ourselves.
...
Now our component is completely independent of the Change Detection, and it does not
