...
Since these are architectural principles, they could be applied in other technologies as well.
...
principles could apply for your application as well. The principles explained in this article are called:
- Smart and Ui components
- Sandbox facades

These principles are explained in this article because they have one thing in common: We use them to enforce
...
It doesn't know about the data services that are fetching the data from our apis, nor does it know about how the application handles global state management. 
...
- It should notify their parent with new events, but it should not care what happens with those events.
...
Even though this kind of component can contain such a complexity, the *month view* should never fetch data itself. That would be the responsibility of the smart components.
...
- We can optimise our application for performance regarding Change Detection

In the tweet below you see that we have created an ebook about Change Detection.
The book is free until December 1st 2022, so get your copy if you still can.
...
Smart components have a big responsibility there: **They orchestrate, they know about the codebase!** This means they consume different
...
When we put our kid in a sandbox in that garden, they know they can play with little buckets, shovels, other
toys that their sandbox has. The responsibility of our kid is now limited and he or she is safe, and can prosper by doing what he
or she is meant to do. We as a parent, shouldn't worry about them taking our lawn mower or heg saw.
...
Smart components shouldn't even know how state is being managed in the codebase, and they definitely shouldn't know which state management framework the codebase uses.
...
The goal is that the feature library can not consume **anyhing** unless it is exposed through its sandbox facade.
...
- It can send actions on a ngrx store but the method name should not reveal if we are using ngrx.
...
- It makes developers think what code they can expose and what code they shouldn't expose
